...
class DataProto:
    ...
    def custom_repeat_by_counts(self, repeat_counts, interleave=True):
        """
        Repeat each sample in the batch according to repeat_counts.

        Args:
            repeat_counts (Sequence[int]):
                长度必须等于当前 batch size（len(self)）。
                第 i 个元素表示第 i 条样本要重复多少次。
            interleave (bool):
                目前只支持 True（和 repeat_interleave 行为一致）。
        Returns:
            DataProto: 新的 DataProto，batch_size = sum(repeat_counts)
        """
        # 转成 list，避免传进来的是 numpy / tensor
        repeat_counts = list(repeat_counts)
        assert len(self) == len(repeat_counts), \
            f"repeat_counts length {len(repeat_counts)} != batch size {len(self)}"
        assert all(c >= 0 for c in repeat_counts), "repeat_counts must be non-negative"
        assert interleave, "custom_repeat_by_counts currently only supports interleave=True"

        new_batch_size = (sum(repeat_counts),)

        # ====== 1. 处理 tensor batch ======
        if self.batch is not None:
            if len(self.batch.keys()) > 0:
                # 正常情况：有若干个 tensor key
                # 不强依赖 device，从每个 tensor 自己拿 device
                repeated_tensors = {}
                repeats_tensor_cpu = torch.as_tensor(repeat_counts)  # 先在 CPU 上
                for key, tensor in self.batch.items():
                    repeats_tensor = repeats_tensor_cpu.to(tensor.device)
                    repeated_tensors[key] = tensor.repeat_interleave(repeats_tensor, dim=0)
            else:
                # 特殊情况：batch 是空 TensorDict（没有任何 key），但有 batch_size
                # 那就构造一个“空”的 TensorDict，只更新 batch_size
                repeated_tensors = {}
            repeated_batch = TensorDict(
                source=repeated_tensors,
        ...
        # ====== 2. 处理 non_tensor_batch（numpy + object） ======
        repeated_non_tensor_batch = {}
        for key, arr in self.non_tensor_batch.items():
            repeated_list = []
            for item, c in zip(arr, repeat_counts):
                if c > 0:
                    repeated_list.extend([item] * c)
