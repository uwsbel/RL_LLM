...
class RayPPOTrainer(object):
    ...
    """
    def _validate(self):
        print("Starting validation...,by Jingquan")
        reward_tensor_lst = []
        data_source_lst = []
        ...
        for test_data in tqdm(self.val_dataloader, desc="Validation"):
            ...
            if self.config.actor_rollout_ref.rollout.name in ['vllm_multi_turn', 'vllm_multi_turn_via_chat']:
                print("使用多轮对话rollout for validation")
            ...
            # 如果是多轮对话，需要还原 test_batch
            if self.config.actor_rollout_ref.rollout.name in ['vllm_multi_turn', 'vllm_multi_turn_via_chat']:
                turn_count_list = test_output_gen_batch.non_tensor_batch['dialogue_turns'].tolist()
                # 和 fit() 里一样的还原逻辑
                ...
                # 确保长度对得上
                assert len(consolidated_turn_count) == len(test_batch.batch), \
                    f"consolidated_turn_count len {len(consolidated_turn_count)} != test_batch size {len(test_batch.batch)}"

                # 展开 test_batch，让它和 test_output_gen_batch batch_size 一致
                test_batch = test_batch.custom_repeat_by_counts(
                    repeat_counts=consolidated_turn_count,
                    interleave=True,
                )
    ...
    def _validate(self):
        ...
        for test_data in tqdm(self.val_dataloader, desc="Validation"):
            ...
            # 只在 rule-based rm 上做 val
            if self.config.reward_model.enable and \
            test_batch[0].non_tensor_batch['reward_model']['style'] == 'model':
                return {}

            # 在 pop 之前先把 input_ids 保存下来
            orig_input_ids = test_batch.batch['input_ids']          # [B, L]
            base_input_texts = [
                self.tokenizer.decode(ids, skip_special_tokens=True)
                for ids in orig_input_ids
            ]  # 长度 = B（对话数）
            ...
            # ========= 这里处理 multi-turn 的展开 & logging 对齐 =========
            if self.config.actor_rollout_ref.rollout.name in ['vllm_multi_turn', 'vllm_multi_turn_via_chat']:
                turn_count_list = test_output_gen_batch.non_tensor_batch['dialogue_turns'].tolist()
                ...
                # 展开 tensor / non-tensor 到轮级
                test_batch = test_batch.custom_repeat_by_counts(
                    repeat_counts=consolidated_turn_count,
                    interleave=True,
                )

                # === logging 用的 input_texts 也要展开到轮级 ===
                input_texts = []
                idx = 0
                for c in consolidated_turn_count:
                    # 第 idx 个对话的文本，复制 c 次
                    input_texts.extend([base_input_texts[idx]] * c)
                    idx += 1
            else:
                # 单轮情况：直接用原始 B 条 input
                input_texts = base_input_texts

            # 把这次 batch 的 input texts 记录下来
            sample_inputs.extend(input_texts)
        ...
        for i in range(reward_tensor.shape[0]):
            data_source = data_sources[i]
            data_source_reward.setdefault(data_source, []).append(
                reward_tensor[i].item()
            )

        metric_dict = {
            f'val/test_score/{ds}': np.mean(rewards)
