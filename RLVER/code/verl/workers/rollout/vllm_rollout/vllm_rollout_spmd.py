...
class vLLMMultiTurnViaChatRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto,**kwargs)->DataProto:#, environment, **kwargs) -> DataProto: #see verl/single_controller/base/decorator l.54, we can't send these classes as usual.
        ...
        if vllm_version in ('0.3.1', '0.4.2', '0.5.4', '0.6.3') and self.config.free_cache_engine:
            self.inference_engine.init_cache_engine()
        def _debug_dump_prompts(prompts, max_show: int = 1):
            import numpy as np
            print("\n====== [prompts.debug] DUMP START ======")

            # ---- batch 区 ----
            try:
                print("[batch] keys:", list(prompts.batch.keys()))
                for k, v in prompts.batch.items():
                    try:
                        print(f"  batch[{k}]: shape={tuple(v.shape)}, dtype={v.dtype}, device={getattr(v, 'device', None)}")
                    except Exception as e:
                        print(f"  batch[{k}]: <print failed: {e}>")
            except Exception as e:
                print(f"[batch] <no batch or print failed: {e}>")

            # ---- non_tensor_batch 区 ----
            ntb = getattr(prompts, "non_tensor_batch", None)
            print("[non_tensor_batch] exists:", isinstance(ntb, dict))
                ...
                for k, v in ntb.items():
                    try:
                        if isinstance(v, np.ndarray):
                            elem_type = type(v.flat[0]) if v.size else None
                            print(f"  ntb[{k}]: numpy, shape={v.shape}, dtype={v.dtype}, elem0_type={elem_type}")
                        else:
                            print(f"  ntb[{k}]: type={type(v)}")
                    except Exception as e:
                        print(f"  ntb[{k}]: <print failed: {e}>")

                # 给几个常用字段打个样例
                if 'raw_prompt' in ntb:
                    try:
                        rp = ntb['raw_prompt']
                        sample = rp[0, 0] if isinstance(rp, np.ndarray) and rp.size else (rp[0] if isinstance(rp, (list, tuple)) else None)
                        print("  sample raw_prompt[0]:", sample)
                    except Exception as e:
                        print("  sample raw_prompt[0] print failed:", e)

                if 'simulator' in ntb:
                    sim = ntb['simulator']
                    if isinstance(sim, np.ndarray):
                        print("  simulator ndarray shape:", sim.shape,
                            "elem0_type:", (type(sim.flat[0]) if sim.size else None))
                    else:
                        print("  simulator type:", type(sim))

                if 'simulator_cfg' in ntb:
                    cfg = ntb['simulator_cfg']
                    if isinstance(cfg, np.ndarray):
                        print("  simulator_cfg ndarray shape:", cfg.shape,
                            "elem0_type:", (type(cfg.flat[0]) if cfg.size else None))
                    else:
                        print("  simulator_cfg type:", type(cfg))

            # ---- meta_info 区 ----
            mi = getattr(prompts, "meta_info", {}) or {}
            try:
                print("[meta_info] keys:", list(mi.keys()))
                # 常见会关心的一两个项
                if 'stage' in mi:
                    print("  stage:", mi['stage'])
                if 'do_sample' in mi:
                    print("  do_sample:", mi['do_sample'])
            except Exception as e:
                print(f"[meta_info] <print failed: {e}>")

            print("====== [prompts.debug] DUMP END ======\n")
        _debug_dump_prompts(prompts)
